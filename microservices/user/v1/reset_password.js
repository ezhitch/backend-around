...
router.post('/', function(req, res) {
    ...
    User.retrieveRecord(registration_phone_number).then(function (record) {
	    if (record === null) {
		    ...
		    logger.debug('Registered user reset password, verify authcode which user inputs');
		    Redis.client.get(registration_phone_number, function (err, authcode) {
			    ...
			    if (req.body.authcode !== authcode) {
				    ...
				    logger.debug('Handling reset password');
			    }

			    User.resetPassword(req.body).then(function (isSuccess) {
				    if (isSuccess) {
					    sender.sendUserResetPasswordSuccess(res);
				    } else {
					    sender.sendUserResetPasswordFailed(res);
				    }
			    });
		    });
	    }
