...
router.post('/', function(req, res) {
	...
	User.isLogin(registration_phone_number).then(function (isLogin) {
		...
		logger.debug('Handling delivery address set default address');
		...
		DeliveryAddress.isRecordFound(registration_phone_number, address_number).then(function (isFound) {
			if (!isFound) {
				...
				sender.sendDeliveryAddressRecordSetDefaultAddressFailed(res);
				return;
			}

			logger.debug('Record [', registration_phone_number, ', ', address_number, '] is found, set default address');
			DeliveryAddress.setDefaultAddress(req.body).then(function (isSuccess) {
				if (isSuccess) {
					sender.sendDeliveryAddressRecordSetDefaultAddressSuccess(res);
				} else {
					sender.sendDeliveryAddressRecordSetDefaultAddressFailed(res);
				}
			});
