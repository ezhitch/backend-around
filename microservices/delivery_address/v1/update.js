...
router.post('/', function(req, res) {
	...
	User.isLogin(registration_phone_number).then(function (isLogin) {
		...
		logger.debug('Handling delivery address update');
		...
		DeliveryAddress.isRecordFound(registration_phone_number, address_number).then(function (isFound) {
			...
			logger.debug('Record [', registration_phone_number, ', ', address_number, '] is found, update towards database');
			DeliveryAddress.updateRecord(req.body).then(function (isSuccess) {
				if (isSuccess) {
					sender.sendDeliveryAddressRecordUpdateSuccess(res);
				} else {
					sender.sendDeliveryAddressRecordUpdateFailed(res);
				}
			});
		});
	}).catch(function (err) {
