"use strict";
exports.__esModule = true;
let request = require("request-promise-native");

request({
	uri: 'http://localhost:443/delivery_address/v1/set_default_address',
	method: 'POST',
	headers: {
		'Accept': 'application/json',
		'Content-Type': 'application/json',
		'uuid': '1E2DFA89-496A-47FD-9941-DF1FC4E6484A',
		'token': ''
	},
	body: {
		registration_phone_number: '13800138000',
		address_number: 1,
		is_default_address: 1 // can be 0 to cancel default address, app needs to support this feature
	},
	json: true,
	resolveWithFullResponse: true
}).then(function (response) {
	console.log('##########Received Response##########');
	console.log('HEADERS BEGIN: ');
	console.log(response.headers);
	console.log('\n');
	console.log('BODY BEGIN: ');
	console.log(response.body);
})["catch"](function (error) {
	console.log('Error: ' + error.toString());
});
